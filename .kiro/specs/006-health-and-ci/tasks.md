...
- [ ] 1.1 Create `app/routes/health.py`
...
- [ ] 1.2 Verify the file compiles: `python -c "from app.routes.health import router"`
...
- [ ] 2.1 Open `app/main.py`
- [ ] 2.2 Import: `from app.routes import health` and `from app.routes.health import set_ready`
- [ ] 2.3 Register the health router — no tags:
  ```python
  app.include_router(health.router)
  ```
- [ ] 2.4 In the lifespan handler, add `set_ready(True)` immediately after `setup_otel()` completes
- [ ] 2.5 In the lifespan shutdown section, add `set_ready(False)` as the first line before `shutdown_otel()`
- [ ] 2.6 Update `FastAPIInstrumentor.instrument_app()` call to exclude health paths:
  ```python
...
- [ ] 3.1 Add tests to `tests/test_routes.py` (or create `tests/test_health.py`):
...
- [ ] 3.2 Import `set_ready` in the test file and use monkeypatch to control the flag — do not
  start a real lifespan
- [ ] 3.3 Run: `OTEL_SDK_DISABLED=true pytest tests/ -v` — must pass with 138+ tests
...
- [ ] 4.1 Create `pyproject.toml` at the repo root (if it does not already exist)
- [ ] 4.2 Add ruff configuration:
  ```toml
  [tool.ruff]
  line-length = 100
  select = ["E", "F", "I", "UP"]
...
- [ ] 5.1 Open `.github/workflows/ci.yml` (currently empty)
- [ ] 5.2 Write the workflow:
  ```yaml
  name: CI
      ...
      steps:
        ...
        - uses: actions/upload-artifact@v4
          ...
          with:
            ...
            path: results.xml
  ```
- [ ] 5.3 Confirm: no `OPENAI_API_KEY` secret is referenced anywhere in this workflow
...
- [ ] 6.1 Open `.github/workflows/regression.yml` (currently empty)
- [ ] 6.2 Write the workflow:
  ```yaml
  name: Regression
  ...
  ```
- [ ] 6.3 Confirm: no `OPENAI_API_KEY` secret referenced. Eval runners mock the gateway.
...
- [ ] 7.1 `OTEL_SDK_DISABLED=true pytest tests/ -v` — all 138+ tests pass
- [ ] 7.2 `ruff check app/routes/health.py` — zero errors
- [ ] 7.3 `mypy app/routes/health.py` — zero errors
- [ ] 7.4 `python -c "import yaml; yaml.safe_load(open('.github/workflows/ci.yml'))"` — valid YAML
- [ ] 7.5 `python -c "import yaml; yaml.safe_load(open('.github/workflows/regression.yml'))"` — valid YAML
- [ ] 7.6 Manual: start the app and confirm both endpoints respond correctly without auth
...
- `OTEL_SDK_DISABLED=true pytest tests/ -v` reports ≥ 138 tests, all passing
